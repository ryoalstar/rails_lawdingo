      ...
      $.ajax(
        ...
        success: (response) ->
          if response.result
            ...
            $('span#subscription_price').text('$' + parseInt(stripe_plan_amount - stripe_plan_amount*response.coupon.percent_off*0.01) + '/month for the first one month, $'+stripe_plan_amount+' thereafter period.')
          else
